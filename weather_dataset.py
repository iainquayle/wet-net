...
class WeatherDataset(Dataset):
	def __init__(self, root, set_names, matching_file_names, sequence_size=1, truth_sequence_size=1, truth_offset=0, stride=1):
		super().__init__() 
		self.root = root
	...
	@staticmethod
	def new_from_files(root, set_names, sequence_size=1, truth_sequence_size=1, truth_offset=0, stride=1):
		...
		return WeatherDataset(root, set_names, matching_file_names, sequence_size, truth_sequence_size, truth_offset, stride)
	def split_set(self, ratio):
		new_set = WeatherDataset(self.root, self.set_names, self.matching_file_names[int(len(self.matching_file_names) * ratio):], self.sequence_size, self.truth_sequence_size,self.truth_offset, self.stride)
		self.matching_file_names = self.matching_file_names[:int(len(self.matching_file_names) * ratio)]
		return new_set	
	def __len__(self):
		return len(self.matching_file_names)  - (self.sequence_size - 1 + self.truth_offset + self.truth_sequence_size - 1) * self.stride
	def __getitem__(self, index):
		images = []
		for i in range(self.sequence_size):
			image = cat([read_image(join(self.root, set_name, self.matching_file_names[index + (i * self.stride)])) for set_name in self.set_names])
			images.append(self.transforms(image / 255.0).masked_fill_(self.mask, 0.0))
		truth_images = []
		for i in range(self.truth_sequence_size):
			image = cat([read_image(join(self.root, set_name, self.matching_file_names[index + ((self.sequence_size - 1 + self.truth_offset + i) * self.stride)])) for set_name in self.set_names])
...
PLOT_RESIZE = 512 
def plot_image_pairs(images):
	fig = plt.figure(figsize=(15, 10))
	for index, (image, truth) in enumerate(images):
		image = functional.resize(image.detach(), 512) 
		truth = functional.resize(truth, 512) 
		fig.add_subplot(2, len(images), index + 1)
		plt.imshow(torch.split(squeeze(image), 3)[0].permute(1, 2, 0))
		plt.axis('off')
		fig.add_subplot(2, len(images), len(images) + index + 1)
		plt.imshow(torch.split(squeeze(truth), 3)[0].permute(1, 2, 0))
		plt.axis('off')
